 ...
 */
package com.yang.exam.commons.ipseeker;
...
public class IPSeeker {
        ...
        for (long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
            ...
            if (temp != -1) {
                ...
                if (ipLoc.country.indexOf(s) != -1 || ipLoc.area.indexOf(s) != -1) {
                    ...
                    // 得到起始IP
                    readIP(offset - 4, b4);
                    entry.beginIp = IPSeekerUtil.getIpStringFromBytes(b4);
                    // 得到结束IP
                    readIP(temp, b4);
                    entry.endIp = IPSeekerUtil.getIpStringFromBytes(b4);
                    // 添加该记录
                    ret.add(entry);
                }
            }
    ...
    public String getCountry(byte[] ip) {
        ...
        if (ipFile == null)
            return "数据库损坏";
        // 保存ip，转换ip字节数组为字符串形式
        String ipStr = IPSeekerUtil.getIpStringFromBytes(ip);
    ...
    public String getCountry(String ip) {
        return getCountry(IPSeekerUtil.getIpByteArrayFromString(ip));
    }
    ...
    public String getArea(String ip) {
        return getArea(IPSeekerUtil.getIpByteArrayFromString(ip));
    }
    ...
    private String readString(long offset) {
        try {
            ...
            for (i = 0, buf[i] = ipFile.readByte(); buf[i] != 0; buf[++i] = ipFile.readByte())
                ;
            if (i != 0)
                return IPSeekerUtil.getString(buf, 0, i, "GBK");
        } catch (IOException e) {
            log.error(e.getMessage());
        }
        return "";
